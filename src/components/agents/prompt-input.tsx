...
> {
  ...
  loading?: boolean;
  onStop?: () => void;
  onSteer?: (value: string, model?: string) => void | Promise<void>;
  stopButtonClassName?: string;
  minRows?: number;
  maxRows?: number;
  leadingAction?: ReactNode;
  /** Local change: replaces the built-in model select when the parent draws its own picker. */
  modelSlot?: ReactNode;
  /** Local change: drawn after the model picker, before the send button. The panel puts the spend here. */
  trailing?: ReactNode;
  /** Local change: the textarea, for a parent that wants to focus it. */
...
export function PromptInput({
  ...
  const promptReady = Boolean(currentValue.trim()) && !disabled;
  const canSubmit = promptReady && !loading;
  const canSteer = promptReady && loading && onSteer !== undefined;
  const action: "send" | "steer" | "stop" = loading ? (canSteer ? "steer" : "stop") : "send";
  ...
  const submit = (event?: FormEvent) => {
    ...
    if (loading && onSteer === undefined) return;

    if (loading) {
      onSteer?.(prompt, currentModelValue);
    } else {
    ...
    >
      ...
      <div className="mt-1 flex min-h-8 items-center gap-1">
        ...
        <Button
          ...
          size="icon"
          disabled={action === "send" ? !canSubmit : action === "stop" && !onStop}
          aria-label={action === "send" ? "Send prompt" : action === "steer" ? "Steer turn" : "Stop generating"}
          onClick={action === "steer" ? () => submit() : action === "stop" ? onStop : undefined}
          className={cn("ml-auto size-8 rounded-full", action === "stop" && stopButtonClassName)}
        >
